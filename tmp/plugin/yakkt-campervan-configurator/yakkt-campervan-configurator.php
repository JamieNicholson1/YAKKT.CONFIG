...
function yakkt_add_configured_item_to_cart($request) {
    ...
    // Check if this is a direct API call (not from an AJAX request)
    $is_direct_request = !empty($_SERVER['HTTP_USER_AGENT']) && 
                       strpos($_SERVER['HTTP_USER_AGENT'], 'Mozilla') !== false;
    
    yakkt_log("Request type: " . ($is_direct_request ? "Direct browser request" : "API/AJAX request"));
    
    // If this is a direct browser request, use the token approach instead
    if ($is_direct_request) {
        yakkt_log("Direct browser request detected - generating token for redirect flow");
        
        // Generate a unique token
        $token = wp_generate_password(32, false);
        
        // Store the request data in a transient
        set_transient('yakkt_config_' . $token, $request->get_json_params(), 5 * MINUTE_IN_SECONDS);
        
        // Redirect to our special endpoint with the token
        ...
        yakkt_log("Redirecting to: " . $redirect_url);
    ...
    // Original code for API/AJAX requests continues below
    ...
    $components_by_category = array();
    
    if (!empty($components)) {
        foreach ($components as $component) {
            if (isset($component['name'])) {
                // Add to category groups if category is set
                $category = isset($component['category']) ? sanitize_text_field($component['category']) : 'Other';
                if (!isset($components_by_category[$category])) {
                    $components_by_category[$category] = array();
                }
                ...
                $components_by_category[$category][] = array(
                    ...
                    'description' => isset($component['description']) ? sanitize_text_field($component['description']) : ''
                );
                
                // Also keep a flat list for meta
                $component_details_for_meta[] = array(
                    'id' => isset($component['id']) ? sanitize_text_field($component['id']) : 'N/A',
                    'name' => sanitize_text_field($component['name']),
                    'price' => isset($component['price']) ? floatval($component['price']) : 0,
    ...
    $yakkt_custom_data = array(
        ...
        '_yakkt_components_by_category' => $components_by_category,
...
function yakkt_display_custom_cart_item_name($product_name, $cart_item, $cart_item_key) {
    if (isset($cart_item['_yakkt_is_configured']) && $cart_item['_yakkt_is_configured'] === true) {
        ...
        $chassis_name = isset($cart_item['_yakkt_chassis_name']) ? 
            esc_html($cart_item['_yakkt_chassis_name']) : 'Custom Van';
            
        // Start with the van model
        $output = $chassis_name . ' Configuration';
        
        // Add component details by category in simple text format
        if (isset($cart_item['_yakkt_components_by_category']) && !empty($cart_item['_yakkt_components_by_category'])) {
            $output .= "\n\n";
            
            foreach ($cart_item['_yakkt_components_by_category'] as $category => $components) {
                if (!empty($components)) {
                    ...
                    foreach ($components as $component) {
                        $price_text = isset($component['price']) && $component['price'] > 0 ? 
                            sprintf(" - £%.2f", floatval($component['price'])) : "";
                            
                        $output .= "• " . esc_html($component['name']) . $price_text . "\n";
                    }
                    
                    $output .= "\n";
                }
            }
        } elseif (isset($cart_item['_yakkt_components']) && !empty($cart_item['_yakkt_components'])) {
            // Fallback to flat list if category grouping is not available
            $output .= "\n\nSelected Components:\n";
            
            foreach ($cart_item['_yakkt_components'] as $component) {
                $price_text = isset($component['price']) && $component['price'] > 0 ? 
                    sprintf(" - £%.2f", floatval($component['price'])) : "";
...
function yakkt_add_custom_data_to_order_item($item, $cart_item_key, $values, $order) {
    if (isset($values['_yakkt_is_configured']) && $values['_yakkt_is_configured'] === true) {
        ...
        if (isset($values['_yakkt_chassis_name'])) {
            $item->add_meta_data('Van Model', $values['_yakkt_chassis_name'], true);
        }
        
        // Add components by category for better organization
        if (isset($values['_yakkt_components_by_category']) && is_array($values['_yakkt_components_by_category'])) {
            // First add a total count of components
            $total_components = 0;
            foreach ($values['_yakkt_components_by_category'] as $components) {
                $total_components += count($components);
            }
            $item->add_meta_data('Total Components', $total_components, true);
            
            // Then add each category
            foreach ($values['_yakkt_components_by_category'] as $category => $components) {
                if (!empty($components)) {
                    $category_name = ucwords(str_replace('-', ' ', $category));
                    $components_string = "";
                    ...
                    foreach ($components as $component) {
                        ...
                        $components_string .= sprintf("• %s%s\n", esc_html($component['name']), $price_text);
                    }
                    
                    $item->add_meta_data($category_name, $components_string, true);
                }
            }
        } elseif (isset($values['_yakkt_components']) && is_array($values['_yakkt_components'])) {
            // Fallback to flat list if category grouping is not available
            $components_string = "";
            ...
            $item->add_meta_data('Components', $components_string, true);
        }
        
        // Add total configured price
        if (isset($values['_yakkt_total_price'])) {
            $item->add_meta_data('Total Configured Price', wc_price($values['_yakkt_total_price']), true);
        }
    }
}

/**
 * Hide quantity selector completely for configured items in cart (as they should always be quantity 1)
 */
...
function yakkt_cart_item_quantity($product_quantity, $cart_item_key, $cart_item) {
    if (isset($cart_item['_yakkt_is_configured']) && $cart_item['_yakkt_is_configured'] === true) {
        // Return plain text '1' with no controls
        return '<span class="yakkt-fixed-quantity">1</span>';
    }
    return $product_quantity;
}

/**
 * Add formatted component info to cart item data display
 * This is used in mini-cart and other places where the full product name isn't displayed
 */
add_filter('woocommerce_get_item_data', 'yakkt_add_component_data_to_cart_item', 20, 2);
function yakkt_add_component_data_to_cart_item($item_data, $cart_item) {
    if (isset($cart_item['_yakkt_is_configured']) && $cart_item['_yakkt_is_configured'] === true) {
        // Add chassis info
        if (isset($cart_item['_yakkt_chassis_name'])) {
            $item_data[] = array(
                'key'   => 'Model',
                'value' => esc_html($cart_item['_yakkt_chassis_name'])
            );
        }
        
        // Add component count
        if (isset($cart_item['_yakkt_components']) && is_array($cart_item['_yakkt_components'])) {
            $item_data[] = array(
                'key'   => 'Components',
                'value' => count($cart_item['_yakkt_components'])
            );
        }
        
        // Display all components grouped by category
        if (isset($cart_item['_yakkt_components_by_category']) && is_array($cart_item['_yakkt_components_by_category'])) {
            foreach ($cart_item['_yakkt_components_by_category'] as $category => $components) {
                if (!empty($components)) {
                    $category_name = ucwords(str_replace('-', ' ', $category));
                    
                    // Format each component on a new line with a bullet point
                    $component_list = [];
                    foreach ($components as $component) {
                        $price_text = isset($component['price']) && floatval($component['price']) > 0 ? 
                            sprintf(" (£%.2f)", floatval($component['price'])) : "";
                        $component_list[] = "• " . esc_html($component['name']) . $price_text;
                    }
                    
                    $item_data[] = array(
                        'key'   => $category_name,
                        'value' => implode('<br>', $component_list)
                    );
                }
            }
        } else if (isset($cart_item['_yakkt_components']) && is_array($cart_item['_yakkt_components']) && !empty($cart_item['_yakkt_components'])) {
            // Fallback if no category grouping
            $component_list = [];
            foreach ($cart_item['_yakkt_components'] as $component) {
                $price_text = isset($component['price']) && floatval($component['price']) > 0 ? 
                    sprintf(" (£%.2f)", floatval($component['price'])) : "";
...
function yakkt_handle_direct_add() {
    ...
    if (isset($_GET['yakkt_action']) && $_GET['yakkt_action'] === 'add_to_cart' && isset($_GET['token'])) {
        ...
        // Process each component, with special handling for component names
        foreach ($components as $component) {
            if (isset($component['name'])) {
                // Pre-process names that might need to be split into separate lines
                $name = sanitize_text_field($component['name']);
                $category = isset($component['category']) ? sanitize_text_field($component['category']) : 'Other';
                ...
                // If it contains certain keywords like "Front Runner", check if it needs to be split
                if ($category === 'roof-rack-accessories' || $category === 'rear-door-accessories') {
                    // Create a separate component for display purposes
                    $components_by_category[$category][] = array(
                        'id' => isset($component['id']) ? sanitize_text_field($component['id']) : 'N/A',
                        'name' => $name,
                        'price' => isset($component['price']) ? floatval($component['price']) : 0,
        ...
        // Create a flat list of components for meta storage
        $component_details_for_meta = array();
        foreach ($components_by_category as $category => $category_components) {
            foreach ($category_components as $component) {
                $component_details_for_meta[] = array_merge($component, array('category' => $category));
            }
        }
        
        // Prepare custom cart item data
        $yakkt_custom_data = array(
            '_yakkt_config_summary' => "Configured Van: " . $chassis_name,
            '_yakkt_total_price'    => $total_price,
            '_yakkt_chassis_id'     => $chassis_id,
            '_yakkt_chassis_name'   => $chassis_name,
            '_yakkt_components'     => $component_details_for_meta,
        ...
        wc_add_notice(sprintf('Added "%s" to your cart.', "Configured " . $chassis_name), 'success');
...
/**
 * Add custom CSS for configurator items in cart
 */
add_action('wp_head', 'yakkt_add_cart_item_styles');
function yakkt_add_cart_item_styles() {
    // Only add styles on cart and checkout pages
    if (is_cart() || is_checkout()) {
        ?>
        <style type="text/css">
            /* Configurator item styling */
            .woocommerce-cart-form__cart-item:has(.yakkt-fixed-quantity) td.product-quantity {
                text-align: center;
            }
            .yakkt-fixed-quantity {
                font-weight: bold;
            ...
            .woocommerce dl.variation dt:first-of-type {
                margin-top: 0;
            }
            .woocommerce dl.variation dt:not(:first-of-type) {
                border-top: 1px solid #f0f0f0;
                padding-top: 8px;
            }
            
            /* Ensure each item in a list is on its own line */
            .woocommerce dl.variation dd p br {
                display: block;
                content: "";
                margin-top: 5px;
            }

            /* Fix for Roof Rack Accessories display */
            .woocommerce dl.variation dd p {
                position: relative;
                padding-left: 15px;
                display: block;
            ...
            .woocommerce dl.variation dd p:before {
                content: "•";
                position: absolute;
                left: 0;
                top: 0;
            ...
            /* Don't show bullet on empty lines */
            .woocommerce dl.variation dd p:empty:before {
                content: none;
            }

            /* Fix for stacked items - make each one a block element */
            .woocommerce-cart-form__cart-item dl.variation dd p {
                display: block !important;
                margin-bottom: 4px !important;
            }
            
            /* Additional spacing between categories */
            .woocommerce dl.variation dt + dd {
                margin-top: 5px;
            }
        </style>
        <?php
...
/**
 * Format the display of component data in the review order section of checkout
 */
add_filter('woocommerce_checkout_cart_item_quantity', 'yakkt_format_checkout_item_quantity', 10, 3);
function yakkt_format_checkout_item_quantity($quantity_html, $cart_item, $cart_item_key) {
    if (isset($cart_item['_yakkt_is_configured']) && $cart_item['_yakkt_is_configured'] === true) {
        return ''; // Hide quantity in checkout review
    }
    return $quantity_html;
}

/**
 * Ensure product thumbnail displays correctly in cart and checkout
 */
add_filter('woocommerce_cart_item_thumbnail', 'yakkt_format_cart_item_thumbnail', 10, 3);
function yakkt_format_cart_item_thumbnail($thumbnail, $cart_item, $cart_item_key) {
    if (isset($cart_item['_yakkt_is_configured']) && $cart_item['_yakkt_is_configured'] === true) {
        // If there's no thumbnail, use a default van image or the default WooCommerce placeholder
        if ($thumbnail === '') {
            $placeholder_src = wc_placeholder_img_src();
            return '<img src="' . esc_url($placeholder_src) . '" alt="Configured Van" class="yakkt-van-thumbnail" />';
        }
    }
    return $thumbnail;
}
...
/**
 * Add a note in the order details that this is a configured van
 */
add_action('woocommerce_before_order_itemmeta', 'yakkt_add_configured_van_note', 10, 3);
function yakkt_add_configured_van_note($item_id, $item, $product) {
    // Check if this is our configured product
    if ($item->get_meta('Van Model')) {
        echo '<div class="yakkt-configured-note" style="margin-bottom:10px;font-style:italic;color:#666;">Custom configured van with selected components.</div>';
    }
} 